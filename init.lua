...
do
  ...
  -- Disable netrw's file explorer: nvim-tree.lua requires it (`:help nvim-tree-netrw`),
  -- otherwise both explorers claim the same directories. Must run before plugins load.
  vim.g.loaded_netrw = 1
  vim.g.loaded_netrwPlugin = 1

  -- Must be set before plugins load, otherwise they pick up the wrong leader.
  vim.g.mapleader = ' '
  vim.g.maplocalleader = ' '

  vim.g.have_nerd_font = true

  -- Scheduled after `UiEnter`, because setting `clipboard` eagerly costs startup
  -- time. Drop this line if the OS clipboard should stay independent of Neovim.
...
do
  ...
  require('which-key').setup {
    ...
    --
    -- A `group` entry only supplies a label for a prefix; the individual
    -- bindings show up underneath it automatically from their own `desc`. So
    -- this list needs an entry for every prefix that has NO direct mapping of
    -- its own - `<leader>g` for example is a pure namespace for `gb`/`gB`,
    -- while `<leader>e` is itself a mapping and needs nothing here.
    spec = {
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(event)
      ...
      if client and client:supports_method('textDocument/inlayHint', event.buf) then
        -- `<leader>ti`, not `<leader>th`: the terminal openers in
        -- `lua/custom/plugins/term.lua` own `<leader>t{h,v,t}`.
...
do
  ...
  -- Tell every LSP server about blink.cmp's extra completion capabilities
  -- (`:help blink-cmp-config-capabilities`). `get_lsp_capabilities` already
  -- includes Neovim's defaults; the deep-extend states that explicitly so no
...
-- ============================================================
-- SECTION 10: LOCAL PLUGINS
-- loads lua/custom/plugins/*.lua
-- ============================================================
do
  -- Loads every `lua/custom/plugins/*.lua` except `init.lua` itself. Files load
  -- in `vim.fs.dir()` order, which is unspecified - so keep mutually dependent
  -- plugins in one file, with their `vim.pack.add()` / `setup()` calls in the
  -- order they need to run.
